        ...
        this.g().selectAll(".node").data(nodes, function (d) {
            ...
            .call(function () {
                var transform = function (d) {
                    ...
                    fill = function (d) {
                        return d.state() === "stopped" ? "gray" : "steelblue";
                    },
                    stroke = {
                        dash: function (d) {
                            return (d.state() === "learner" ? "5,5" : "");
                        },
                        opacity: function (d) {
                            return (d.state() === "acceptor" || d.state() === "stopped" ? 0 : 1);
                        },
                    };
                ...
                g.select("circle")
                    .attr("r", function (d) {
                        return self.parent().scales.r(d.r);
                    })
                    .style("stroke-dasharray", stroke.dash)
                    .style("stroke-opacity", stroke.opacity)
                    .style("fill", fill);
                g.select("text.node-value")
                    .attr("font-size", function (d) {
                        return self.parent().scales.font(12)
                    })
                    .text(function (d) {
                        return d.value();
                    });
                g.select("text.node-description")
                    ...
                    .attr("font-size", function (d) {
                        return self.parent().scales.font(8)
                    });
                ...
                g.each(function (node) {
                    ...
                    d3.select(this).select("g.log").selectAll("g.log-entry").data(node.log())
                        .call(function () {
                            var transform = function (d) {
                                return "translate(" + self.parent().scales.size(d.dx) + "," + self.parent().scales.size(d.dy) + ")"
                            };
                            var text = {
                                x: function (d) {
                                    return self.parent().scales.size(0.25);
                                },
                                y: function (d) {
