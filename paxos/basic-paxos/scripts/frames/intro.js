    ...
    return function (frame) {
	   ...
	   .after(1000, function () {
            frame.model().title = '<h2>Paxos is a algorithm for implementing distributed consensus.</h2>'
                        + '<h3 id="paxosDesc" style="visibility:hidden;">Paxos is often used to implement distributed state machines.</h3>'
                        + '<h3 id="smDesc" style="visibility:hidden;">State machine can be database, configuration center, naming service, or any business program.</h3>'
                        + '<br/>' + frame.model().controls.html();
            layout.invalidate();
        })
        .after(1000, function () {
            layout.fadeIn($(".title #paxosDesc"));
        })
        .after(1000, function () {
            layout.fadeIn($(".title #smDesc"));
        })
		.after(1000, function () {
            frame.model().controls.show();
        })
